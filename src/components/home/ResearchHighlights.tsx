        ...
        <div className="grid items-end gap-8 md:grid-cols-[1fr_auto]">
          <div>
            ...
            <p className="mt-5 max-w-xl text-primary-foreground/80">
              EWU researchers publish in top international venues and partner with national
              institutions to solve problems that matter at home, from rural healthcare to climate
              resilience.
            </p>
          </div>
          <Link
        ...
        <div className="mt-16 grid gap-6 rounded-2xl border border-primary-foreground/15 bg-primary-foreground/5 px-6 py-10 sm:grid-cols-4 backdrop-blur">
          {stats.map((s) => (
            <div key={s.label} className="text-center">
              <p className="font-serif text-3xl font-bold text-white/80 md:text-4xl">
                <CountUp end={s.value} prefix={s.prefix} suffix={s.suffix} />
              </p>
              <p className="mt-2 text-[11px] uppercase tracking-wider text-primary-foreground/70">
                {s.label}
              </p>
            </div>
          ))}
